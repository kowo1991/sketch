...
%
\xjmd{For the multilayer perceptrons in the entire networks, we use paramertes mlp(8,16,64), mlp(64,16,2) in PN-1, mlp(32,32,128),mlp(128,64,29) in PN-2, mlp(64,128,1024),mlp(512,256,250) in PN-3.}
%
...
As shown, our algorithm achieves comparable performance \xjmd{$74.22\%$} with image-based convolutional networks.
%
...
\begin{figure}[htbp]
    ...
    \fcaption{Illustration of recognition successes (the first row) and failures (the second row). The most top predicting scores are on the right side of each sketch. \cxj{revise this figure.}}
    \label{fig:resshow}
\end{figure}
